  ...
  const loadPreferences = async () => {
    try {
      const {
        data: classData,
        error: classError
      } = await supabase.from("classes").select("name, allow_gender_preference").eq("id", classId).single();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const {
        ...
        allow_gender_preference: preferences.mix_genders_at_tables
      }).eq("id", classId);
        ...
        <Card className="shadow-[var(--shadow-glow)]">
          ...
          <CardContent className="space-y-6">
            ...
            <div className="flex items-start justify-between gap-4 py-3 border-b">
              ...
              <Switch id="mixGenders" checked={preferences.mix_genders_at_tables} onCheckedChange={checked => setPreferences({
              ...preferences,
        ...
        <Card className="shadow-[var(--shadow-glow)]">
          ...
          <CardContent className="space-y-8">
            ...
            <div className="space-y-4">
              ...
              {preferences.must_sit_together.length === 0 ? <p className="text-sm text-muted-foreground italic py-2">No pairings added yet</p> : <div className="space-y-3">
                  {preferences.must_sit_together.map(pairing => <div key={pairing.id} className="flex items-center gap-2 p-3 rounded-lg bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-800">
            ...
            <div className="space-y-4">
              ...
              {preferences.must_not_sit_together.length === 0 ? <p className="text-sm text-muted-foreground italic py-2">No pairings added yet</p> : <div className="space-y-3">
                  {preferences.must_not_sit_together.map(pairing => <div key={pairing.id} className="flex items-center gap-2 p-3 rounded-lg bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800">
